...
class EsAddrSerializer(serializers.Serializer):
...
class EsBaseInstanceSerializer(serializers.Serializer):
    address = EsAddrSerializer(help_text=_("ip:port 形式的实例地址"))
...
class EsHostClusterOutputSerializer(serializers.Serializer):
    """ES拓扑输入序列化器"""
...
class EsBatchTopoInputSerializer(serializers.Serializer):
    """ES批量拓扑输入序列化器"""

    hosts = serializers.ListField(child=serializers.CharField(), help_text=_("主机IP地址列表"), required=False)
    instances = serializers.ListField(child=EsTopoInputSerializer(), help_text=_("ES实例列表"), required=False)
...
class EsInstancesOutputSerializer(serializers.Serializer):
    ...
    ports = serializers.CharField(help_text=_("实例端口"))


class EsInstancesSummarySerializer(serializers.Serializer):
    instances = serializers.ListSerializer(child=EsInstancesOutputSerializer(), help_text=_("存储层实例汇总"))
...
class StorageInstancesTopoSerializer(serializers.Serializer):
    ...
    by_role = serializers.DictField(
        child=serializers.IntegerField(), help_text=_("按角色分布统计，如 {'es_master': 56, 'es_client': 56}")
    )
...
class EsEmptyInputSerializer(serializers.Serializer):
    userid = serializers.CharField(help_text=_("占位符"))


class EsClustersOutputSerializer(serializers.Serializer):
    bk_cloud_id = serializers.IntegerField(help_text=_("云区域ID"))
    cluster_type = serializers.CharField(help_text=_("集群类型"))
    cluster_id = serializers.IntegerField(help_text=_("集群ID"))
    immute_domain = serializers.CharField(help_text=_("集群域名"))
    ...
    region = serializers.CharField(help_text=_("地域"))
    master_nodes_count = serializers.IntegerField(help_text=_("master节点数"))
    hot_nodes_count = serializers.IntegerField(help_text=_("热节点数"))
    cold_nodes_count = serializers.IntegerField(help_text=_("冷节点数"))
    client_nodes_count = serializers.IntegerField(help_text=_("client节点数"))
